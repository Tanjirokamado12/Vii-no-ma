...
const processFile = async () => {
  ...
  for await (const line of rl) {
    ...
    if (key && value) {
      ...
      // Process when the 'pop' field is encountered (indicating the end of the movie entry)
      if (key === 'pop' && currentMovie['movieid']) {
        const { movieid, title, genre, staff } = currentMovie;

        if (movieid && title && genre && staff && !processedMovieIds.has(movieid)) {
          if (lineIndex < outputFilePaths.length) {
            const movieMeta = createMovieMeta(movieid, title, genre, staff);
