...
function generateXml(genre, movies) {
    let xmlContent = `<GenrePopularMovies>\n`;
    xmlContent += `  <ver>1</ver>\n`;
    xmlContent += `  <genre>${genre}</genre>\n`; // Use genre as a number
...
function saveXmlFiles(movieData) {
    Object.keys(movieData).forEach(typeKey => {
        ...
        const outputDir = path.join(__dirname, '../../v512/url1/list/popular/'); // Output directory
        ensureDirectoryExists(outputDir); // Ensure the output directory exists
        const fileName = `0${genreNumber}.xml`; // Create file name
...
// Main execution
const inputFile = path.join(__dirname, '../../files/v512/GenrePopularMovies.txt');
