...
const fileStream = fs.createReadStream('../files/v512/GenrePopularMovies.txt');
...
<MovieMeta>
  ...
  <dsdist>0</dsdist>
  <staff>0</staff>
  <upddt>2035-04-23T16:30:00</upddt>
</MovieMeta>`;
};
...
const processFile = async () => {
  ...
  for await (const line of rl) {
    ...
    if (key === 'type') {
      currentType = value; // Capture the type (genre number)
    } else if (key === 'movieid' || key === 'title') {
      currentMovie[key] = value;

      if (currentMovie['movieid'] && currentMovie['title'] && currentType) {
        const { movieid, title } = currentMovie;
        const genre = currentType; // Use the type directly as the genre number

        if (!processedMovieIds.has(movieid) && lineIndex < outputFilePaths.length) {
