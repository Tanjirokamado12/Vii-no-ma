...
async function saveXMLFile(movie) {
  try {
    ...
    if (!filePath) {
      console.error(`No file path found for movie ID: ${movie.movieid}`);
      return;
    }
    ...
    console.error(`Error saving XML file for movie ID ${movie.movieid}:`, err);
  }
}
...
async function processMovies() {
  try {
    ...
    console.log("All movies have been processed successfully.");
  } catch (err) {
    console.error("Error during movie processing:", err);
  }
}
