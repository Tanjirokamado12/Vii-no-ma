...
async function processDeliveryEntries() {
    const sourceFilePath = path.join(__dirname, '../../files/v1025/Couponagree.txt');
    ...
    try {
        ...
        if (!deliveryEntries) {
            error('No delivery entries found in the source file.');
            return;
        }
        ...
        for (const entry of deliveryEntries) {
            ...
            const targetFilePath = path.join(__dirname, `../../v1025/url1/Coupon/${deliveryId}.xml`);
