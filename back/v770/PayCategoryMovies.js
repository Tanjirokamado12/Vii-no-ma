...
const inputFilePath = path.join(__dirname, '../../files/v770/PayCategoryMovies.txt');
const outputDir = path.join(__dirname, '../../v770/url3/pay/list/category/search');
...
fs.readFile(inputFilePath, 'utf8', (err, data) => {
    ...
    xmlFiles.forEach(file => {
        ...
        fs.writeFile(outputFilePath, file.xmlContent, (err) => {
            if (err) {
                console.error(`Error writing XML file for categid ${file.categid}:`, err);
            } else {
            }
        });
