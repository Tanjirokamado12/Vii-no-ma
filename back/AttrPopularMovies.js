...
fs.readFile(inputFilePath, 'utf8', (err, data) => {
  ...
  const xml = xmlbuilder.create('AttrPopularMovies', { headless: true })
    .ele('ver', 1).up()
    .ele('type', 1).up();
  ...
  fs.mkdir(outputDir, { recursive: true }, (err) => {
    ...
    // Generate XML files with the same information
    for (let i = 0; i < 14; i++) {
      const fileNumber = (i < 9) ? `0${i + 1}` : `${i + 1}`;
      const outputFilePath = path.join(outputDir, `${fileNumber}.xml`);
