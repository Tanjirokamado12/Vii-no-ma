...
while (currentDate <= endDate) {
  ...
  // Add the update date
  weekContent += `  <upddt>2009-04-23T16:30:00</upddt>\n`;
  weekContent += `</Calendar>\n`;

  // Determine file name: "This.xml" for the current week or standard file name
  const now = new Date();
  const isCurrentWeek = now >= addDays(currentDate, -7) && now <= currentDate; // Check if now falls within this week
  const fileName = isCurrentWeek ? "This.xml" : `${weekStartDate}.xml`;
