...
async function copyFiles(sourceFile, destinationDir, subDir, file, suffix) {
  try {
    ...
    (`Copied: ${sourceFile} -> ${destinationFile}`);
  } catch (err) {
    (`Error copying ${sourceFile}: ${err.message}`);
  }
}
...
async function copyAndRenameMovies() {
  for (const fileName of fileNames) {
    ...
    try {
      // Copy .jpg file first, regardless of movie files' existence
      ...
      if (!movieExists) {
        (`Skipping ${file}, no movie file found.`);
        continue;
      }
